...
export class AutoReplyPage {
  ...
  async goto() {
    ...
    await this.page.locator('a[href="/auto-reply"]').click();
    await this.page.getByRole('heading', { name: '自動リプライ' }).waitFor({ timeout: 10_000 });
  }

  get heading() {
    return this.page.getByRole('heading', { name: '自動リプライ' });
  }
  ...
  get emptyState() {
    return this.page.getByText('自動リプライルールがありません');
  }

  /** ルールカード内のToggle（button要素）。data-testid で特定 */
  toggleButton(index: number): Locator {
    return this.page.locator(`[data-testid="auto-reply-toggle-${index}"]`);
  }
  ...
  get templateInput(): Locator {
    return this.page.getByPlaceholder('自動送信する返信のテンプレートを入力');
  }

  get tweetIdInput(): Locator {
    return this.page.getByPlaceholder('ポストのURLまたはツイートIDを入力');
  }
